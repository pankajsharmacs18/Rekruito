...
const Header = () => {
  ...
  const navigationItems = {
    ...
    admin: [
      ...
      {
        label: "Admin Panel",
        path: "/admin-moderation-management",
        icon: "Shield",
      },
      {
        label: "Analytics",
    ...
    anonymous: [
      ...
      {
        label: "Sign In",
        path: "/job-seeker-registration-login",
        icon: "LogIn",
      },
    ],
  };
  ...
  const currentNavItems = isAuthenticated
    ? navigationItems[userRole]|| navigationItems["job-seeker"]
      ...
      "job-seeker": [
        {
          label: "Saved Jobs",
          icon: "Bookmark",
          action: () => console.log("Saved Jobs clicked"),
        },
        {
          label: "Application History",
          icon: "Clock",
          action: () => console.log("Application History clicked"),
        },
      ],
      recruiter: [
        {
        ...
        },
        {
          label: "Job Analytics",
          icon: "BarChart3",
          action: () => console.log("Job Analytics clicked"),
        },
      ],
      admin: [
        {
                    ...
                    <div className="py-2">
                      {getUserMenuItems().map((item, index) => (
                        <button
                          key={index}
                          onClick={item.action}
                          className={`w-full flex items-center space-x-3 px-4 py-2 text-sm text-left hover:bg-surface transition-smooth ${
                            item.className || ""
                          }`}
                        >
                          <Icon name={item.icon} size={16} />
